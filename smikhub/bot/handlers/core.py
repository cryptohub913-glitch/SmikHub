...
async def admin_broadcast(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(text, reply_markup=kb_back("admin"))
...
async def topup_cryptobot(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(text, reply_markup=kb_back("topup_menu"))
...
async def process_crypto_amount(message: types.Message, state: FSMContext):
    try:
        ...
        return
    await state.clear()
    usdt_amount = max(0.1, round(amount / 100.0, 2))
    invoice_url = None
    if CRYPTO_BOT_TOKEN:
        try:
    ...
    if invoice_url:
        ...
        await message.answer(f"🧾 Запрос на {amount:.2f} RUB создан.\n\nШлюз ожидает подтверждения.", reply_markup=kb_back("topup_menu"))
...
async def process_stars_amount(message: types.Message, state: FSMContext):
    ...
    try:
        await message.answer_invoice(
            ...
            description=f"Начисление {rub_val} RUB на баланс",
            payload=f"stars_{message.from_user.id}_{rub_val}",
            currency="XTR",
            prices=prices
        )
    except Exception as e:
        await message.answer(f"⚠️ Ошибка создания счёта: {e}", reply_markup=kb_back("topup_menu"))
...
async def withdraw_sendpay(callback: types.CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    balance = float(user.balance) if user and user.balance else 0.0
    if balance < 100.0:
        text = f"⚠️ Недостаточно средств\n\nТекущий баланс: {balance:.2f} RUB.\nМинимум для вывода: 100.00 RUB."
        await callback.message.edit_text(text, reply_markup=kb_back("withdraw_menu"))
        return
...
async def process_withdraw_requisites(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    if float(user.balance) < amount:
        await message.answer("⚠️ Ошибка: недостаточно средств.", reply_markup=kb_main_menu())
        return
    user.balance -= Decimal(str(amount))
    await session.commit()
    await message.answer(
        f"✅ Заявка на вывод #{secrets.token_hex(4)} принята!\n\n• Сумма: {amount:.2f} RUB\n• Реквизиты: {requisites}\n• Статус: В обработке (SendPay)",
...
async def nav_add_bot(callback: types.CallbackQuery, session: AsyncSession):
    ...
    await callback.message.edit_text(text, reply_markup=kb_back("sell_traffic"))
...
async def bot_manage(callback: types.CallbackQuery, session: AsyncSession):
    ...
    if not bot_obj:
        await callback.message.edit_text("Бот не найден.", reply_markup=kb_back("sell_traffic"))
        return
...
async def bot_delete(callback: types.CallbackQuery, session: AsyncSession):
    ...
    if bot_obj:
        ...
        await session.commit()
    await callback.message.edit_text("🗑 Бот удалён из системы.", reply_markup=kb_back("sell_traffic"))
...
async def nav_create_order(callback: types.CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    balance = float(user.balance) if user and user.balance else 0.0
    if balance < 100.0:
        text = f"⚠️ Недостаточно средств\n\nБаланс: {balance:.2f} RUB (минимум 100 RUB).\nПополните баланс в Кабинете."
        await callback.message.edit_text(text, reply_markup=kb_back("buy_traffic"))
        return
...
async def nav_referrals(callback: types.CallbackQuery):
    ...
    text = f"🤝 Партнёрская программа SmikHub\n\n• 5% от дохода 1-го уровня\n• 2% от дохода 2-го уровня\n\n🔗 Ваша ссылка:\n{link}"
